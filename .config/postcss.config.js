...
module.exports = ({ file, options, env }) => {
  const variablesFactory = typeof options.variables === 'function' ? options.variables() : options.variables;
  const mediaFactory = typeof options.media === 'function' ? options.media() : options.media;
  const mixinsFactory = typeof options.mixins === 'function' ? options.mixins() : options.mixins;
  return {
    plugins: [
      ...
      require('postcss-mixins')({ mixins: mixinsFactory.mixins(variablesFactory.variables) }),
      ...
      require('postcss-custom-properties')({ preserve: false, variables: variablesFactory.variables }),
      ...
      require('postcss-lh')({ lineHeight: parseInt(variablesFactory.variables.d_lh) / 10, unit: 'vr' }),

      // W3C CSS Custom Media Queries, e.g. @custom-media --small-viewport (max-width: 30em);
      // https://github.com/postcss/postcss-custom-media
      require('postcss-custom-media')({ extensions: mediaFactory.media(variablesFactory.variables) }),
